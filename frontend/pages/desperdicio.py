...
if not lista_comidas:
    ...
    with aba_listar:
        ...
        try:
            resp_desp = requests.get(API_DESPERDICIOS)
            if resp_desp.status_code == 200 and resp_desp.json():
                desperdicios = resp_desp.json()
                df = pd.DataFrame(desperdicios)
                
                if termo_busca:
                    df = df[df["setor"].str.lower().str.contains(termo_busca)]
    ...
    with aba_editar:
        st.subheader("Modificar um registro existente")
        try:
            resp_desp = requests.get(API_DESPERDICIOS)
            if resp_desp.status_code == 200 and resp_desp.json():
                desperdicios_edit = resp_desp.json()
    ...
    with aba_deletar:
        ...
        try:
            resp_desp = requests.get(API_DESPERDICIOS)
            if resp_desp.status_code == 200 and resp_desp.json():
                desperdicios_del = resp_desp.json()

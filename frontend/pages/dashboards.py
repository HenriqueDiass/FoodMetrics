...
if resposta.status_code == 200:
    ...
    if dados:
        ...
        df_mostrar = df[["id", "Produto", "quantidade", "setor", "motivo", "custo_estimado"]].copy()
        df_mostrar.columns = ["ID", "Produto", "Peso (Kg)", "Setor", "Motivo", "Custo (R$)"]
        
        # --- TRUQUE DO ALINHAMENTO À ESQUERDA ---
        df_mostrar["Peso (Kg)"] = df_mostrar["Peso (Kg)"].apply(lambda x: f"{float(x):.2f}")
